...
>
> Обновлён 28 июл 2026 — тотальный код-ревью (C++ + Lua)
...
│   ├── heck.hpp       # ГЛАВНЫЙ ХЕДЕР (1506 строк): все классы
...
1. **Гигантский хедер** — `heck.hpp` 1506 строк, в нём всё: классы, структуры, реализации inline методов. Нет разделения на `.hpp`/`.cpp` для каждого класса. `heck.cpp` тоже огромен.
...
6. **renderGame рекурсия** — `renderGame()` вызывает `nextNode()` → `renderGame()` → ... через клик. Стек Lua может переполниться на длинной сессии. Замыкания на `currentPage` — ещё одна проблема: колбэк держит устаревший `currentPage` при множественных вызовах.
...
11. **Путь к шрифту захардкожен** — `"assets/HackRegular-gX84.ttf"` в четырёх местах (sscreen.lua, game.lua (×2), settings.lua) + дефолтный `/usr/share/fonts/TTF/DejaVuSans.ttf` в heck.cpp:67.
...
13. **`NOLINTBEGIN` на весь хедер** — строки 4 и 1506 отключают clang-analyzer, bugprone, performance на 1506 строк кода. Реальные баги не ловятся.
14. **`BatchKey` сравнивает указатели на `VertexLayout`** — если создать два одинаковых layout в разных местах, они не будут батчиться вместе, т.к. сравнение идёт по адресу, а не по содержимому.
15. **Все текстуры/звуки прелоадятся при старте** — `preloadTextures("assets")` грузит всё подряд, включая неиспользуемый `bal.png` (1.3MB). Нет ленивой загрузки.
16. **Половина коммитов — CI fix** — 38% git истории (34 из 90) — это починка GitHub Actions. Разработчик не тестировал локально.
...
────────────────────────────────────────────────
ИТОГО:      90 коммитов| ~45 часов    ||  среднее 2.9 ч/активный день
```
...
Всего **90 коммитов** (на момент код-ревью). Автор: `ImpostorBoy`.
...
`heck.hpp:1303` — `Sigma &operator=(Sigma &&) = default;`
...
`heck.hpp:883-889` — `Image::onResize` и `Rectangle::onResize` вызывают `setHitbox` после ресайза. `Text::onResize` — нет. Текстовые кнопки имеют неправильную зону клика после изменения размера окна.

### HIGH: Deadlock в preloadTextures при большом количестве потоков
`heck.hpp:184-193` — `std::async(std::launch::async, ...)` в цикле по файлам. Каждый вызов создаёт **новый OS thread**. Если в `assets/` много файлов — потенциально сотни тредов. Thread pool не используется.
...
`bind.hpp:128` — `luaState.set_function("sllep", [&](int secs) { sleep(secs); });`
...
### MEDIUM: appendGlyphs тихо отбрасывает глифы при переполнении атласа
`heck.hpp:748` — если шрифт не влезает в `MAX_ATLAS_SIZE=2048`, `growAtlasToFit` возвращает `false`, и глифы не рендерятся без какого-либо уведомления.
...
### LOW: BatchKey сравнивает указатели на VertexLayout
`heck.hpp:229-257` — `b.key.layout == key.layout` — сравнение указателей, а не содержимого. Два одинаковых layout, созданные в разных местах, не будут батчиться вместе.

### LOW: JohnPork::getOrCreate не копирует ключ для новых батчей
`heck.hpp:256` — `return &batches.emplace_back();` — новый батч создаётся с key по умолчанию, key присваивается только на следующем `pushGeometry` (строка 268). Если между `reserve` и `pushGeometry` произойдёт реаллокация `batches`, указатель станет невалидным.
...
`game.lua:401-408` — `while id and id ~= vn.currentNode do ... id = n.next end`
`script.json` содержит цикл: узел "7" → `"next": "1"`. Если `currentNode` находится после цикла или недостижим от `start`, цикл никогда не завершится — **гарантированный зависон** при загрузке сохранения.

### HIGH: Notification timer игнорирует 0.5 секунды
`game.lua:370` — `notifTimer = os.time() + 0.5`
`os.time()` возвращает **integer** секунд. 0.5 отбрасывается: нотификация живёт либо 0 сек, либо 1 сек (когда `os.time()` переключится). Не полсекунды.
...
### HIGH: sload() и initSload() — 98% дубликат (35 строк копипасты)
`game.lua:373-418` и `570-604` — две почти идентичные функции загрузки. Если исправить одну и забыть про другую — рассинхронизируются.

### HIGH: syncSound() — запутанная логика раннего возврата
`game.lua:272-301` — `if node.sound == nil then if not vn.currentSound or vn.currentSoundId ~= 0 then return end`. Если звук уже играет (`currentSoundId != 0`) и в узле нет `sound` — функция молча выходит. Но если звук закончился (`currentSoundId == 0`), она **перезапускает** старый. Логика неочевидна, поведение зависит от гонки `currentSoundId`.
...
### MEDIUM: Игрок застревает на последней ноде
`game.lua:551` — `if currentPage < #pages or node.next then`. У ноды "5" в `script.json` нет поля `next`. На последней странице — нет кнопки продолжения. Выход только через F2/F3/закрытие окна.

### MEDIUM: menuOnKey и settingsOnKey не определены
...
`game.lua:272-296` — после `sload()` или `initSload()` `vn.currentSoundId` сбрасывается в 0, а `vn.currentSound` восстанавливается из цепочки нод. Если у текущей ноды нет `sound` — `syncSound` перезапускает старый звук заново.

### MEDIUM: Гонка currentPage
`game.lua:529,554-564` — В `renderGame()` создаётся локальная `local currentPage = math.min(vn.currentPage, #pages)`. Клик-колбэк замыкается на эту локальную переменную. Если `renderGame()` вызвать несколько раз (через resize), колбэк держит устаревший `currentPage`.

### LOW: Функция center() определена дважды
`game.lua:446-454` — `function center(elementSize, containerSize)` — два одинаковых определения. Второе перезаписывает первое. Мёртвый код.

### LOW: ssave() содержит мёртвый I/O
`game.lua:354-367` — комментарий `-- inflicts segfault(no more)` намекает что была затычка от segfault'а, которую забыли убрать. Функция пишет только новую data, старый read мёртв.
...
### Мёртвый код
- `heck.hpp:42` — `//static bool fuckCpp = true;` закомментированный глобал
- `game.lua:15-18` — таблица `local saved` никогда не используется (осталась от слотов сохранений)
- `game.lua:33` — `g.textOk` нигде не используется
- `bind.hpp:128` — `sllep()` не вызывается ни в одном скрипте
- `game.lua:446-454` — `function center()` определена дважды, второе определение перезаписывает первое
...
### getTextGooner() может бросить исключение через Lua
`bind.hpp:81` — `throw std::runtime_error(...)` при неудачной загрузке шрифта. Исключение пробрасывается через sol2 в Lua, где ловится в `main.cpp`. Но ни один Lua-скрипт не ожидает ошибку — если шрифт не загрузится, игра упадёт.

### UploadAtlasRegion — копирование построчно
`heck.hpp:711-714` — подпроизводительное копирование региона атласа построчно вместо одного `memcpy` строки целиком. Для маленьких регионов ок, но для больших — лишние вызовы.

### shaderc + bgfx tools (80 MB) закоммичены в git
В `external/bgfx/tools/bin/linux/` лежат бинарники (shaderc, texturec, libdxcompiler.so) общим размером ~80 MB. Они в git, что раздувает репозиторий.

### .gitignore не покрывает артефакты сборки и профилирования
`perf.data` (538 KB), `heaptrack.*.zst`, `.build/*.o`, `.build/build.ninja`, `.build/.ninja_*`, `gcm.cache/` — всё это в git. Репозиторий раздут.
...
| Коммитов CI fix | 34 (38%) — `fix gh ci v1..16`, `fix make v1..8`, `fix gh ci 2..5` |
| CIL (Commits In Life) | 90+ |
...
```
Из 90 коммитов **38% — это CI** (34 коммита). Разработчик потратил почти половину времени на борьбу с GitHub Actions.

### Драма с сохранениями
`0c1696e we are fucking removing save slots` — в ранней версии были слоты сохранений (вероятно, несколько JSON/SLOT файлов). Разработчик выпилил мульти-слоты, оставив один `state.json`. Код всё ещё содержит следы: мёртвая таблица `local saved` в game.lua и закомментированные структуры данных.

### Коммит-призрак
`308ab05 very useful commit` — без какого-либо контекста. Полезный коммит без описания, что в нём полезного. Разработчик забыл написать, что делал.

### Doxygen? Не, не слышали
Ни одного комментария в стиле Doxygen. Все комментарии — либо TODO, либо инлайн-пояснения, либо русский мат. Единственный документирующий файл — этот (AI-generated).
...
**Результат код-ревью (28 июл 2026):**
- **3 бага CRITICAL** — бесконечный цикл при загрузке (sload), отсутствие хитбокса у текста при ресайзе, double-free в Sigma move-assignment
- **7 багов HIGH** — битый таймер нотификации, shadowing переменной saved, 35 строк копипасты sload/initSload, запутанная логика syncSound, сброс прогресса при resize, зависание на последней ноде, утечка TextureHandle
- **~15 багов MEDIUM/LOW** — мёртвый код, гонка currentPage, переполнение uint16 в батчах, O(n²) поиск в JohnPork, signed overflow, и т.д.

**Вердикт:** забавно, работает, но глаза кровоточат. 7/10.  
**Срочно пофиксить:** бесконечный цикл в sload() и хитбоксы текста при ресайзе.
